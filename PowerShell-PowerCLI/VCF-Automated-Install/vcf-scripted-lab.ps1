...
# Changes:
# added protocol and force to connect to vcenter
# added force to deploy ova with import vapp
# Change to VMs: MGT Port on esxi, subnet to 252 on all VMs
# Make sure all VMs and Hosts are in DNS!!!!
# NTP: used time.google.com
...
if($deployNestedESXiVMsForMgmt -eq 1) {
    $NestedESXiHostnameToIPsForManagementDomain.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
        ...
        $a = $ovfconfig
        $a | Select-Object *
        Write-Host "OVF Config:"$ovfconfig
        ...
        if($vmPortGroup.NetworkType -eq "Distributed") {
            ...
            #Set-NetworkAdapter -VM $vm -NetworkAdapter "Network adapter 1" -Connected $false
            #Set-NetworkAdapter -NetworkAdapter "Network adapter 2" -Connected $true
        } else {
